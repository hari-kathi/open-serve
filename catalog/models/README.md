...
Enable a model by merging its preset into your environment values under `serveModels:` and setting `enabled: true`; disable it with `enabled: false` (the RayService is removed without touching other models). All presets ship `enabled: false`, `tier: production`, `type: vllm`, and scale-to-zero replicas (`min: 0, max: 1`).
...
- **Embeddings** — served through vLLM's pooling runner (`vllmArgs.runner: pooling`); reach them at `/v1/embeddings` — the gateway routes by the request body's `model` field via `gateway.modelRoutes` (add an entry mapping the model id to its `rayservice-<name>-serve-svc`).
